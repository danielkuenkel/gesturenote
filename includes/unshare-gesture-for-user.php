...
session_start();
if (isset($_SESSION['user_id'], $_POST['gestureId'], $_POST['id'], $_POST['title'], $_POST['email'])) {
        ...
        if (!$delete_stmt->execute()) {
            ...
            if ($select_invited_users_stmt = $mysqli->prepare("SELECT * FROM gestures_shared WHERE gesture_id = '$shareId' AND owner_id = '$sessionUserId'")) {
                if (!$select_invited_users_stmt->execute()) {
                    ...
                    $to = $inviteMail;
                    $subject = $lang->uninviteGestureSubject;
                    $message = '<html>
                                    <head>
                                    <title>' . $lang->uninviteGestureTitle . '</title>
                                    </head>
                                    <body>
                                        <p">' . $lang->hello . ',</p>
                                        <p>' . $_SESSION['forename'] . ' ' . $_SESSION['surname'] . ' ' . $lang->uninviteGestureText . '</p>
