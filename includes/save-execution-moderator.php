...
session_start();
if (isset($_SESSION['user_id'], $_POST['testerId'], $_POST['studyId'], $_POST['data'])) {
    ...
    if ($select_stmt = $mysqli->prepare("SELECT id FROM study_results_evaluator WHERE evaluator_id = '$userId' && tester_id = '$testerId' && study_id = $studyId LIMIT 1")) {
        if (!$select_stmt->execute()) {
            ...
            if ($select_stmt->num_rows == 1) {
                if ($update_stmt = $mysqli->prepare("UPDATE study_results_evaluator SET data = '$executionData' WHERE id = '$id'")) {
                ...
                if ($insert_stmt = $mysqli->prepare("INSERT INTO study_results_evaluator (study_id, evaluator_id, tester_id, data) VALUES ('$studyId','$userId','$testerId','$executionData')")) {
