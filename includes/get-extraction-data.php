...
session_start();
if (isset($_SESSION['user_id'], $_POST['studyId'], $_POST['method'])) {
    $selectStudyId = $_POST['studyId'];
    $sessionUserId = $_SESSION['user_id'];
    $method = $_POST['method'];
    $surveyType = $_POST['surveyType'];
//
//    echo json_encode(array('status' => $method === 'userCentered', 'method' => $method));
//    exit();

    if ($method === 'userCentered') {
    ...
    } else if ($method === 'expertBased') {
        if ($select_stmt = $mysqli->prepare("SELECT general_data FROM studies WHERE id = '$selectStudyId'")) {
            if (!$select_stmt->execute()) {
                echo json_encode(array('status' => 'selectError'));
                exit();
            } else {
                ...
                while ($select_stmt->fetch()) {
                    $decodedResults = json_decode_nice($data, false);
                    if (isset($decodedResults->mapping)) {
                        $mappings = $decodedResults->mapping;
//                        $phases = $decodedResults->phases;

                        foreach ($mappings as $mapping) {

//                            if ($item->format === "identification") {
                            if (isset($mapping->format) && $mapping->format === 'trigger-mapping' && isset($mapping->gestureIds)) {
                                $gestures = $mapping->gestureIds;
                                foreach ($gestures as $gesture) {
                                    $elicitedGestures[] = array('triggerId' => $mapping->triggerId, 'gestureId' => $gesture);
                                }
                            } else if (isset($mapping->format) && $mapping->format === 'gesture-mapping' && isset($mapping->triggerIds)) {
                                $triggers = $mapping->triggerIds;
                                foreach ($triggers as $trigger) {
                                    $elicitedTrigger[] = array('gestureId' => $mapping->gestureId, 'triggerId' => $trigger);
                                }
                            }
//                            }
                        }
