...
<div class="modal-header">
    ...
    <!-- Nav tabs -->
    <ul class="nav nav-pills text-center" id="gesture-catalogs-nav-tab">
...
//        console.log(data);
        ...
        console.log(currentFilterList, currentFilterData);
        $(currentFilterList).empty();
        var index = getCurrentPaginationIndex();//parseInt($('#custom-pager-top .pagination').find('.active').text()) - 1;
        ...
        for (var i = viewFromIndex; i < viewToIndex; i++) {
            ...
            var clone = getCreateStudyGestureListThumbnail(currentFilterData[i], 'favorite-gesture-catalog-thumbnail', 'col-xs-6 col-md-3', null, isGestureAss ? 'panel-success' : null);
    ...
    $('#gesture-catalogs-nav-tab').on('hide.bs.tab', function (event) {
        ...
        switch ($(event.relatedTarget).attr('href')) {
            case '#study-gesture-set':
                $('#study-gesture-set').find('#sort .chosen').attr('id', 'unselected');
                $('#study-gesture-set').find('#gesture-list-container').empty();
                getStudyGestures();
                break;
            case '#gesture-catalog':
                $('#gesture-catalog').find('#sort .chosen').attr('id', 'unselected');
                $('#gesture-catalog').find('#gesture-list-container').empty();
        ...
        getGestureCatalog(function (result) {
            if (result.status === RESULT_SUCCESS) {
                ...
                if (originalFilterData && originalFilterData.length > 0) {
                    ...
                    // show alert, where no data is there
                }
            }
        });
                        ...
                        if (result.status === RESULT_SUCCESS) {
                            getGestureCatalog(function (result) {
                                if (result.status === RESULT_SUCCESS) {
                                    originalFilterData = result.gestures;
                                    currentFilterData = sort();
                                    $(container).trigger('gesture-deleted');
                                }
                            });
                        }
                    });
