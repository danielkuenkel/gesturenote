...
//            };
            ...
            function onAllExternalsLoadedSuccessfully() {
                appendAlert($('#viewWizard'), ALERT_PLEASE_WAIT);
                initPopover();
...
//            }
            ...
            $(window).on('resize', function () {
                if (!$('#viewWizard #pinnedRTC').hasClass('hidden') && $('#viewObserver #column-left').hasClass('rtc-scalable')) {
                    ...
                    var width = $('#viewWizard #column-left').width();
                    var height = 3 / 4 * width;
                    console.log('reset resize', width);
                    $('#viewWizard #video-caller').css({width: width + 'px', height: height + 'px'});
                }
            });

            $(window).scroll(function () {
                updateRTCHeight($('#viewWizard #column-left').width());
                if(peerConnection) {
            ...
            function updateRTCHeight(updateWidth, updateColumn) {
                if ($('#viewWizard #column-left').hasClass('rtc-scalable')) {
                    var scrollTop = $(document).scrollTop();
                    var newHeight = 3 / 4 * updateWidth - scrollTop;
                    var newWidth = 4 / 3 * newHeight;
                    if (newWidth > DRAGGABLE_MIN_WIDTH) {
                        $('#viewWizard #video-caller').css({height: newHeight + 'px', width: newWidth + 'px'});
                    }

                    if (scrollTop > 0) {
                        $('#viewWizard #video-caller').css({opacity: .7});
                    } else {
                        $('#viewWizard #video-caller').css({opacity: 1});
                    }

                    if (updateColumn) {
                        TweenMax.to($('#viewWizard #column-left'), .2, {css: {marginTop: newHeight + 20}});
                    }
                }
            }
            ...
            function renderPhaseStep() {
                ...
                $('#viewWizard').find('#phase-content').empty();
