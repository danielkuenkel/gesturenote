...
//    console.log('save current moderator status');
    ...
    getGMT(function (timestamp) {
        getFinishedStudyPhases(currentPhaseStepId, studyFinished, function (phases) {
            ...
            saveExecutionObserver({studyId: study.id, testerId: study.testerId, data: data, evaluatorId: study.evaluatorId}, function (result) {
                console.log('save execution observer', result, data);
                if (callback) {
                    callback(result);
                }
            });
