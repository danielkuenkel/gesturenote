...
function Questionnaire(options) {
    ...
    this.options.answers = options.answers || null;
    return this;
}
...
Questionnaire.prototype.renderModeratorView = function () {
    ...
    if (data && data.length > 0) {
        if (isPreview) {
            ...
            renderQuestionnaireAnswers(container, data, answers, true);
        } else {
            console.log('render questionnaire');
            renderQuestionnaire(container, data, answers, true);
        }
    } else {
...
Questionnaire.prototype.renderObserverView = function () {
    console.log('render moderator view:', QUESTIONNAIRE.toUpperCase());
